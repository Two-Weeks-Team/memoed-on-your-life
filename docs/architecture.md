...
The Cloudflare Worker independently validates every field before checking a key or dispatching upstream. A SQLite-backed Durable Object atomically reserves worst-case spend and enforces daily, per-flow, per-installation, call-count, and circuit-breaker limits. The provider adapter pins `gpt-5.4-mini`, `store: false`, a 2,000-token output ceiling, bounded non-streaming reads, a 20-second timeout, and at most one retry for 429 or 5xx responses. It uses the Responses API strict JSON Schema contract and rejects refusals, incomplete output, unexpected models, malformed JSON, unknown fields, and out-of-packet citations.
